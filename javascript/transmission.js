...
Transmission.prototype = {
	...
	initialize : function() {
		...
		$(this.prefsDialog).bind('closed',
				$.proxy(this.onPrefsDialogClosed, this));
					...
					if (($(this).index()) > 0) {
						...
						if (confirm('Are you sure you want to download: \n'
							...
							if (url != '') {
								...
								// alert( "cat1: " + cat1[($(this).index())-1] +
								// "cat2: " +cat2[($(this).index())-1] );
		...
		$('#upload_confirm_button').click(
				$.proxy(this.confirmUploadClicked, this));
		...
		if (!isMobileDevice) {
			...
			$('#torrent_container').bind('dragover',
					$.proxy(this.dragenter, this));
			$('#torrent_container').bind('dragenter',
					$.proxy(this.dragenter, this));
	...
	scrollToRow : function(row) {
		...
		var list = $('#torrent_container'), scrollTop = list.scrollTop(), innerHeight = list
				.innerHeight(), offsetTop = row.getElement().offsetTop, offsetHeight = $(
				row.getElement()).outerHeight();
	...
	setSelectedRow : function(row) {
		$(this.elements.torrent_list).children('.selected').removeClass(
				'selected');
		this.selectRow(row);
	},
	...
	selectionChanged : function() {
		...
		this.inspector.setTorrents(this.inspectorIsVisible() ? this
				.getSelectedTorrents() : []);
	...
	callSelectionChangedSoon : function() {
		if (!this.selectionChangedTimer) {
			var callback = $.proxy(this.selectionChanged, this), msec = 200;
	 ...
	 */
	keyDown : function(ev) {
		var handled = false, rows = this._rows, up = ev.keyCode === 38, // up
		// key
		// pressed
		...
		if ((up || dn) && rows.length) {
			var last = this.indexOfLastTorrent(), i = last, anchor = this._shift_index, r, min = 0, max = rows.length - 1;
	...
	drop : function(ev) {
		var i, uri, uris = null, types = [ "text/uri-list", "text/plain" ], paused = this
	...
	togglePeriodicSessionRefresh : function(enabled) {
		...
		if (enabled) {
			var callback = $.proxy(this.loadDaemonPrefs, this), msec = 8000;
	...
	onMenuClicked : function(ev) {
		var o, dir, id = ev.target.id, remote = this.remote, element = $(ev.target);
	...
	updateFromTorrentGet : function(updates, removed_ids) {
		var i, o, t, id, needed, needinfo = [], callback, fields;
								...
								$
										.proxy(
												function(newValue, oldValue) {
													if (oldValue === Torrent._StatusDownload
															&& (newValue == Torrent._StatusSeed || newValue == Torrent._StatusSeedWait)) {
														$(this)
																.trigger(
																		'downloadComplete',
																		[ t ]);
													} else if (oldValue === Torrent._StatusSeed
	...
	updateTorrents : function(ids, fields) {
		this.remote
				.updateTorrents(ids, fields, this.updateFromTorrentGet, this);
	},

	refreshTorrents : function() {
		var callback = $.proxy(this.refreshTorrents, this), msec = this[Prefs._RefreshRate] * 1000, fields = [ 'id' ]
				.concat(Torrent.Fields.Stats);
	...
	onRowClicked : function(ev) {
		var meta_key = ev.metaKey || ev.ctrlKey, row = ev.currentTarget.row;
	...
	uploadTorrentFile : function(confirmed) {
		var i, file, reader, fileInput = $('input#torrent_upload_file'), folderInput = $('input#add-dialog-folder-input'), startInput = $('input#torrent_auto_start'), urlInput = $('input#torrent_upload_url');
	...
	promptSetLocation : function(confirmed, torrents) {
		if (!confirmed) {
			...
			this.remote.moveTorrents(ids, $("input#torrent_path").val(),
	...
	promptToRemoveTorrents : function(torrents) {
		if (torrents.length === 1) {
			var torrent = torrents[0], header = 'Remove ' + torrent.getName()
			...
			var header = 'Remove ' + torrents.length + ' transfers?', message = 'Once removed, continuing the transfers will require the torrent files. Are you sure you want to remove them?';
			dialog.confirm(header, message, 'Remove',
					'transmission.removeTorrents', torrents);
		}
	},
	...
	promptToRemoveTorrentsAndData : function(torrents) {
		if (torrents.length === 1) {
			var torrent = torrents[0], header = 'Remove ' + torrent.getName()
					+ ' and delete data?', message = 'All data downloaded for this torrent will be deleted. Are you sure you want to remove it?';
			dialog.confirm(header, message, 'Remove',
					'transmission.removeTorrentsAndData', torrents);
		} else {
			var header = 'Remove ' + torrents.length
					+ ' transfers and delete data?', message = 'All data downloaded for these torrents will be deleted. Are you sure you want to remove them?';
			dialog.confirm(header, message, 'Remove',
					'transmission.removeTorrentsAndData', torrents);
		}
	},
	...
	hideMobileAddressbar : function(delaySecs) {
		if (isMobileDevice && !scroll_timeout) {
			var callback = $.proxy(this.doToolbarHide, this), msec = delaySecs * 1000 || 150;
	...
	updateGuiFromSession : function(o) {
		var limit, limited, e, b, text, fmt = Transmission.fmt, menu = $('#settings_menu');
	...
	updateStatusbar : function() {
		var u = 0, d = 0, i, row, fmt = Transmission.fmt, torrents = this
				.getAllTorrents();
	...
	updateFilterSelect : function() {
		var i, names, name, str, o, e = $('#filter-tracker'), trackers = this
				.getTrackers();
	...
	updateButtonsSoon : function() {
		if (!this.buttonRefreshTimer) {
			var callback = $.proxy(this.updateButtonStates, this), msec = 100;
			this.buttonRefreshTimer = setTimeout(callback, msec);
		}
	},

	updateButtonStates : function() {
		var e = this.elements, haveActive = false, havePaused = false, haveSel = false, haveActiveSel = false, havePausedSel = false;
	...
	setInspectorVisible : function(visible) {
		...
		if (isMobileDevice) {
			...
			var w = visible ? $('#torrent_inspector').outerWidth() + 1 + 'px'
					: '0px';
	...
	refilter : function(rebuildEverything) {
		var i, e, id, t, row, tmp, rows, clean_rows, dirty_rows, frag, sort_mode = this[Prefs._SortMethod], sort_direction = this[Prefs._SortDirection], filter_mode = this[Prefs._FilterMode], filter_text = this.filterText, filter_tracker = this.filterTracker, renderer = this.torrentRenderer, list = this.elements.torrent_list, old_sel_count = $(
				list).children('.selected').length;
		...
		while (ci != cmax || di != dmax) {
			...
			if (ci == cmax)
				...
				var c = Torrent.compareTorrents(clean_rows[ci].getTorrent(),
						dirty_rows[di].getTorrent(), sort_mode, sort_direction);
				push_clean = (c < 0);
			}
	...
	setFilterTracker : function(domain) {
		...
		var key = domain ? this.getReadableDomain(domain) : 'all', id = '#show-tracker-'
				+ key;
	...
	setCompactMode : function(is_compact) {
		var key = Prefs._CompactDisplayState, was_compact = this[key];
	...
	togglePeriodicStatsRefresh : function(enabled) {
		...
		if (enabled) {
			var callback = $.proxy(this.loadDaemonStats, this), msec = 5000;
	...
	updateStats : function(stats) {
		var s, ratio, fmt = Transmission.fmt;
